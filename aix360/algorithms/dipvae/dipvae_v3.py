...
from .dipvae_utils_v2 import VAE, DIPVAE
from .dipvae_utils_v2 import plot_reconstructions, plot_latent_traversal
...
class DIPVAEExplainer(DIExplainer):
    ...
    def fit(self, visualize=False, save_dir="results"):
        ...
        for key in self.net.optim:
            self.net.optim[key].zero_grad()

        loss_epoch_list = []
        disc_loss_epoch_list=[]
        ...
        for epoch in np.arange(self.model_args.num_epochs):
            ...
            for x, y in self.dataset.next_batch():
                ...
                if "mnist" in self.dataset.name:
                    loss, disc_loss, disc_disent_loss = self.net.neg_elbo_v3(x.squeeze().view(-1, np.prod(self.dataset.data_dims)))
                else:
                    loss, disc_loss, disc_disent_loss = self.net.neg_elbo_v3(x)
                # backward
                loss.backward()
                self.net.optim['g'].step()
                loss_epoch += loss

                self.net.optim['disc_x'].zero_grad()
                disc_loss.backward()
                self.net.optim['disc_x'].step()
            ...
            disc_loss_epoch_list.append(-disc_loss_epoch / self.dataset.num_training_instances)
            print("Epoch {0} | ELBO {1}".format(epoch, -loss_epoch / self.dataset.num_training_instances))
            print("Epoch {0} | Discriminator {1}".format(epoch, -disc_loss_epoch / self.dataset.num_training_instances))
