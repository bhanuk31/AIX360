...
from aix360.algorithms.dipvae.encoders.base.base_encoder import BaseEncoder
from aix360.algorithms.dipvae.decoders.base.base_decoder import BaseDecoder
from aix360.algorithms.dipvae.discriminators.base.base_discriminator import BaseDiscriminator
...
class FCNetEncoder(BaseEncoder):
    def __init__(self, num_nodes=50, ip_dim=1, op_dim=1, activation_type='relu', args=None):
        ...
        if activation_type == 'relu':
            ...
            return
        layer = Linear
        self.feat_input=ip_dim
        self.latent_dim=op_dim
        self.fc_hidden = []
...
class FCNetDecoder(BaseDecoder):
    def __init__(self, num_nodes=50, ip_dim=1, op_dim=1, activation_type='relu', protected, args=None):
        super(FCNetDecoder, self).__init__()
        self.args = args
        if activation_type == 'relu':
            self.activation = nn.ReLU()
        elif activation_type == 'tanh':
        ...
        self.fc_hidden = []
        self.fc1 = layer(ip_dim, num_nodes)
        self.latent_input=self.fc1
        self.protected_input=protected
        self.feat_dim=op_dim
        self.bn1 = nn.BatchNorm1d(num_nodes)
        for _ in np.arange(self.args.num_layers - 1):
            self.fc_hidden.append(layer(num_nodes, num_nodes))
            self.fc_hidden.append(nn.BatchNorm1d(num_nodes))
    ...
    def forward(self, x):
        x = x.squeeze().view(x.size(0), -1)
        x = self.fc1(x)
        x = torch.cat((x, self.protected_input), dim=1)
        x = self.bn1(x)
        x = self.activation(x)
        x = self.features(x)
        return self.fc_out(x)

class FCNetDisc(BaseDiscriminator):
    def __init__(self, num_nodes=50, ip_dim=1, op_dim=1, activation_type='relu', args=None):
        super(FCNetDisc, self).__init__()
        self.args = args
        if activation_type == 'relu':
            self.activation = nn.ReLU()
        elif activation_type == 'tanh':
...
class VAE(nn.Module):
    ...
    """
    def __init__(self, num_nodes=50, latent_dim=10, op_dim=784, activation_type='relu', gan_loss='mse', lamb=1.0, beta=1.0,
                opt=optim.Adam, opt_args={'lr': 0.0002, 'betas': (0.5, 0.999)}, protected_attr=None, args=None, cuda_available=None, init_networks=True):
        super(VAE, self).__init__()
        self.latent_dim = latent_dim
        self.lamb = lamb
        self.beta = beta
        self.protected=protected_attr
        if gan_loss == 'bce':
        ...
        if init_networks:
            ...
            self.generative_net = FCNetEncoder(num_nodes=num_nodes, op_dim=generative_op_dim, ip_dim=latent_dim,
                                        activation_type=activation_type, args=args)
            args.num_layers = args.num_inference_layers
            self.inference_net = FCNetDecoder(num_nodes=num_nodes, op_dim=2 * latent_dim, ip_dim=op_dim,
                                       activation_type=activation_type, protected=protected_attr,
                                       args=args)
            self.disc_x = FCNetDisc(num_nodes=num_nodes, op_dim=1, ip_dim=generative_op_dim,
                                       activation_type=activation_type,
                                       args=args)
        optim_disc_x = opt(filter(lambda p: p.requires_grad,
                                  self.disc_x.parameters()), **opt_args)
        
        optim_g = opt(filter(lambda p: p.requires_grad, self.generative_net.parameters()), **opt_args)
        self.optim={
        'g': optim_g, 
        'disc_x': optim_disc_x
        }
    ...
    def neg_elbo_v2(self, x, beta=None):
        recon, mu_z, std_z, enc = self.forward(x,return_z_sample=True)
        recon = torch.sigmoid(recon)
        recon_loss = torch.mean(torch.abs(recon-x))
        is_2d = True if len(enc.size()) == 2 else False
        perm = torch.randperm(x.size(0))
        alpha = self.sampler(x.size(0), enc.size(1), is_2d)
        enc_mix = alpha*enc + (1.-alpha)*enc[perm]
        dec_enc_mix = self.decode(enc_mix)
        disc_g_mix_loss = self.gan_loss_fn(self.disc_x(dec_enc_mix)[0], 0)
        consist_loss = torch.mean(torch.abs(self.encode(dec_enc_mix)[0]-enc_mix))

        kl_z = self._kl_divergence_z(mu_z, std_z)
        if beta is not None:
            kl_z = beta * kl_z
        d_losses = []
        dx_out, _ = self.disc_x(x)
        d_x_real = self.gan_loss_fn(dx_out, 0)
        d_x_fake = self.gan_loss_fn(self.disc_x(dec_enc_mix.detach())[0],
                                    torch.min(alpha.view(-1), (1-alpha).view(-1)))
        d_losses.append(d_x_real)
        d_losses.append(d_x_fake)
        d_x = sum(d_losses)

        return -self.lamb*(bernoulli_likelihood(x, recon)) + kl_z + disc_g_mix_loss + self.beta*consist_loss, d_x
